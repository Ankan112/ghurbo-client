...
const MyOrders = () => {
    ...
    useEffect(() => {
        fetch(`https://assignment-11-node-mongodb-iyejhgqnc-ankan112.vercel.app/orders/${email}`)
            .then(res => res.json())
    ...
    const handleDeleteOrder = id => {
        ...
        if (proceed) {
            const url = `https://assignment-11-node-mongodb-iyejhgqnc-ankan112.vercel.app/orders/${id}`
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
